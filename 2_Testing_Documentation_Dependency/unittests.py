...
def gr(n, l, f):
    ...
    b = [(i, random.randint(1, 10)) for i in range(1, n) if i not in l]
    c = [
    ...
    [a["0"].append([i, w]) or a[str(i)].append([0, w]) for i, w in b]
    [a[str(i)].append([j, w]) or a[str(j)].append([i, w]) for i, j, w in c]
    [
        a[str(i)].append([j, w])
        for i in l
        for j in range(1, n)
        if j not in l and j != 0
        for w in [random.randint(1, 10)]
    ]
    open(f, "w").write(json.dumps(a, indent=4))
...
def test_dijkstra_fixed(learner_func):
    ...
    def g():
        ...
        tcs = [
            [6, 18, 14],
            [1, 2, 1, 1, 7, 5, 16, 11, 11, 5],
            [17, 8, 18, 1, 13, 17, 10],
            [9, 16, 19, 15, 6, 5, 2, 4, 10, 8, 5],
            [16, 12, 16, 5, 9, 6, 19, 6, 6, 18, 4, 3, 19, 4],
            [2, 1, 7],
            [16, 15],
            [8, 17, 16, 8, 14, 14, 6, 15, 5, 18, 17, 14, 2, 9, 15],
            [],
        ]
    ...
    def g():
        ...
        for attr in attrs:
            if not hasattr(learner_func, attr):
                t.failed = True
                t.msg = (
                    f"{function_name} does not have {attr} method. Aborting unittests"
                )
                return [t]
        ...
        if not success:
            ...
            t.expected = (
                "An Exception of the right type when popping from an empty stack"
            )
...
def test_DoublyLinkedListFixed(learner_func):
    def g():
        ...
        p = multiprocessing.Process(
            target=helper, name="traverse", args=(q, s.traverse)
        )
        p.start()
